...
module.exports = {
      ...
      result.forEach((valueProduct, indexProduct) => {
        ...
        stock.forEach((valueStock) => {
          if (valueStock.id_product === valueProduct.id) {
            detailStock.push({
              idStock: valueStock.id,
              quantity: valueStock.quantity,
              unit: valueStock.unit,
              default_unit: valueStock.default_unit,
            });
          }
        });
  ...
  getProducts: async (req, res, next) => {
    try {
      if (Object.keys(req.query).length == 0) {
        ...
        let allData =
          await dbQuery(`Select p.id, p.name, p.description, p.id_category, c.category_name, s.quantity, s.unit, s.default_unit, p.selling_price, p.buying_price, p.unit_conversion, p.needs_receipt, p.image, s.id as id_stock, p.is_active from products p
  ...
  getDetailProduct: async (req, res, next) => {
    try {
      if (req.params) {
        ...
        let smallestUnit = await dbQuery(
          `Select unit from stock where id_product=${req.params.product_id} and default_unit = 'false'`
        );
      ...
      if (req.dataUser.role === "admin") {
        if (req.body.category_name) {
          let update = await dbQuery(
            `INSERT INTO category (category_name) VALUE ('${req.body.category_name}')`
          );
          if (update) {
            return res.status(200).send({
              success: true,
  ...
  deleteCategories: async (req, res, next) => {
    try {
      if (req.dataUser.role === "admin") {
        if (req.query.id) {
          let productCategory = await dbQuery(
            `Select * from products where id_category = ${req.query.id};`
          );
          if (productCategory.length > 0) {
            ...
            let deleteCategory = await dbQuery(
              `Delete from category where id = ${req.query.id};`
            );
            if (deleteCategory) {
              return res.status(200).send({
                success: true,
  ...
  addProduct: async (req, res, next) => {
    try {
      if (req.dataUser.role === "admin") {
        const uploadFile = uploader("/imgProduct", "IMGPRODUCT").array(
          "image",
          1
        );
  ...
  editProductData: async (req, res, next) => {
    try {
      ...
      let productType = await dbQuery(
        `SELECT * FROM stock WHERE id_product = ${req.query.id}`
      );
      ...
      if (productType.length === 0) {
        ...
        await dbQuery(
          `INSERT INTO stock (id_product, unit, default_unit, quantity) VALUE ${value}`
        );
        ...
        if (add) {
          await dbQuery(
            `INSERT INTO stock (id_product, unit, default_unit, quantity) VALUE ${add}`
          );
        }
        if (update) {
          await dbQuery(
            `UPDATE stock SET ${update} WHERE id = ${productType[0].id}`
          );
        }
        ...
        if (firstUpdate) {
          let initialQuantity = await dbQuery(
            `SELECT quantity FROM stock where id = ${firstId};`
          );
          let finalChange = initialQuantity[0].quantity - firstQuantityChange;
          await dbQuery(
            `INSERT INTO stock_history (id_stock, quantity, type) VALUE (${firstId}, ${finalChange}, 'Stock Update')`
          );
    ...
    try {
      ...
      uploadFile(req, res, async (error) => {
        try {
          ...
          await dbQuery(
            `UPDATE products SET image = ${newFileName} WHERE id = ${req.query.id};`
          );
  ...
  deleteProduct: async (req, res, next) => {
    try {
      if (req.dataUser.role === "admin") {
        if (req.query.id) {
          await dbQuery(
            `UPDATE products SET is_active = 'false' WHERE id = ${req.query.id}`
          );
          return res.status(200).send({
            success: true,
  ...
  unitConversion: async (req, res, next) => {
    try {
      if (req.dataUser.role === "admin") {
        const { productId, stock, conversionInput, conversionResult } =
          req.body;

        const defaultUnitStock = stock[0];
        const conversionUnitStock = stock[1];
        ...
        if (
          updateDefaultUnit.affectedRows &&
          updateConversionUnit.affectedRows
        ) {
          const updateStockHistory = await dbQuery(
            `INSERT INTO stock_history (id_stock, quantity, type) VALUES (${defaultUnitStock.idStock}, ${conversionInput}, 'Unit Conversion'), (${conversionUnitStock.idStock}, ${conversionResult}, 'Unit Conversion')`
          );

          return res.status(200).send({
            success: true,
            message: "Product converted",
          });
        } else {
          return res.status(200).send({
            success: false,
